...
AppSettingsController, UITextFieldDelegate, EmailDelegate, MobileNumberDelegate {
    // Row References
    ...
    func mobileNumberClicked() {
        if WalletManager.sharedInstance().wallet.getTwoStepType() == AuthenticationTwoFactorType.sms.rawValue {
    ...
    @objc func toggleEmailNotifications(_ sender: UISwitch) {
        ...
        if Reachability.hasInternetConnection() {
            if emailNotificationsEnabled() {
                ...
                if WalletManager.sharedInstance().wallet.getEmailVerifiedStatus() == true {
    ...
    func alertUserToChangeTwoStepVerification() {
        ...
        let twoStepType = WalletManager.sharedInstance().wallet.getTwoStepType()
    ...
    func changeTwoStepVerification() {
        ...
        if WalletManager.sharedInstance().wallet.getTwoStepType() == AuthenticationTwoFactorType.none.rawValue {
            isEnablingTwoStepSMS = true
            if WalletManager.sharedInstance().wallet.getSMSVerifiedStatus() == true {
                enableTwoStepForSMS()
            } else {
                mobileNumberClicked()
            }
    ...
    func hasAddedEmail() -> Bool {
        if WalletManager.sharedInstance().wallet.getEmail() != nil {
            return true
        }
        return false
    }
    ...
    func resendVerificationEmail() {
        ...
        WalletManager.sharedInstance().wallet.resendVerificationEmail(emailString)
    }
    ...
    func updateCurrencySymbols() {
        allCurrencySymbolsDictionary = WalletManager.sharedInstance().wallet.btcRates
        reloadTableView()
    }
    ...
    func updateEmailAndMobileStrings() {
        let emailString = WalletManager.sharedInstance().wallet.getEmail()
        if emailString != nil {
            self.emailString = emailString!
        }
        let mobileNumberString = WalletManager.sharedInstance().wallet.getSMSNumber()
