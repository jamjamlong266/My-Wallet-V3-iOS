...
@objc class SettingsTwoStepViewController: UIViewController, SettingsTwoSteps {
    ...
    override func viewDidLoad() {
        ...
        if let aLARGE = UIFont(name: Constants.FontNames.montserratRegular, size: Constants.FontSizes.Large) {
            twoStepButton.titleLabel?.font = aLARGE
        }
    ...
    func updateUI() {
        WalletManager.sharedInstance().wallet.hasEnabledTwoStep() ?
            twoStepButton.setTitle("Disabled", for: .normal) : twoStepButton.setTitle("Enable 2-Step for SMS".localized(), for: .normal)
    }
